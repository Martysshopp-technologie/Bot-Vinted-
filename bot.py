#!/usr/bin/env python3
"""
Vinted Telegram Bot — Mode Polling + Flask health check pour Render (tier gratuit)
"""
...
async def vinted_poll_loop(app: Application):
    refresh_vinted_cookie()
    cleanup_ctr = 0
    while True:
        try:
            ...
            for f in active_filters:
                ...
                for it in new_items[:5]:
                    ...
                    try:
                        ...
                        logger.error(f"Envoi échoué user {f['user_id']}: {e}")
                    await asyncio.sleep(1)
                await asyncio.sleep(2)

        except Exception as e:
            logger.error(f"Erreur polling Vinted: {e}")
...
# ─── FLASK HEALTH CHECK (thread séparé) ───────────────────────────────────────

@flask_app.get("/")
def health():
    return jsonify({"status": "ok", "bot": "Vinted Bot en ligne"})
...
async def main():
    ...
    app.add_handler(CommandHandler("deletefilter", cmd_delete_filter))
    app.add_handler(CommandHandler("pausefilter", cmd_pause_filter))
    app.add_handler(CommandHandler("resumefilter", cmd_resume_filter))
    app.add_handler(CommandHandler("stop", cmd_stop))
    app.add_handler(CommandHandler("resume", cmd_resume))
    app.add_handler(CallbackQueryHandler(btn_callback))

    # Lance Flask dans un thread séparé (health check pour Render)
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    logger.info(f"Serveur Flask lancé sur port {PORT}")

    # Lance le polling Vinted en tâche de fond
    asyncio.create_task(vinted_poll_loop(app))

    logger.info("Bot Vinted démarré en mode polling !")
